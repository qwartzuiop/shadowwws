<template>
    <base-box title="CSS Output">
        <template v-slot:titleAdditional>
            <button
                :class="s.button"
                @click="copyToClipboard(shadowLayers)"
            >
                Copy
                <svg
                    :class="s.buttonImage"
        ...
        <div :class="s.wrapperOutput">
            <pre :class="s.pre">box-shadow: </pre>
            <pre :class="s.pre">{{ shadowLayers }};</pre>
        </div>
    </base-box>
</template>
...
export default {
    ...
    methods: {
        copyToClipboard(textToCopy) {
            ...
            let text = 'box-shadow: ' + textToCopy + ';';
    ...
    computed: {
        shadowLayers() {
            return this.$store.getters.getShadowLayersOutput;
        },
    },
    components: {
        BaseBox,
